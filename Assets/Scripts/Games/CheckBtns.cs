using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.UI;

public class CheckBtns : MonoBehaviour {
    private Passs passs;
    private int indexI;

    public void OnCheck(Transform tf, int index)
    {
        passs = tf.GetComponent<Passs>();
        indexI = index;
    }

    public void CheckClick()
    {
        // 成就
        GameAchStatic.SetAchievement(27);
        GameAchStatic.SetAchievement(28);
        GameAchStatic.SetAchievement(29);

        GameData.Instance.soundOs.PlayCheckSound();
        GameData.Instance.spotCurr = indexI - 1;
        GameData.Instance.passCurr = GameData.Instance.passAdopt + 1;
        //passs.CheckClick(indexI);
        Play();
    }


    public Image cdImg;

    public Sprite[] spArr;

    // 播放速度
    private float bfSpeed = 2f;

    // 当前速度
    private float dqSpeed = 0;

    // 当前播放图片
    private int imgIndex = 0;

    // 隐藏图片
    public void Hide()
    {
        Destroy(transform.gameObject);
        passs.CheckClick();
    }

    // 显示图片
    public void Display()
    {
        cdImg.enabled = true;
    }

    // 刷新图片显示
    private void Refresh()
    {
        cdImg.overrideSprite = spArr[imgIndex];
    }

    // 播放图片
    public void Play()
    {
        transform.GetComponent<Button>().enabled = false;
        imgIndex = 0;
        Refresh();
        //Display();
        StartCoroutine("PlayCheck");
    }

    IEnumerator PlayCheck()
    {
        while (imgIndex < spArr.Length - 1)
        {
            dqSpeed++;
            if (dqSpeed == bfSpeed)
            {
                dqSpeed = 0;
                imgIndex++;
                Refresh();
            }
            yield return 0;
        }
        Hide();
    }
}
